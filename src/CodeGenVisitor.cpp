...
antlrcpp::Any CodeGenVisitor::visitDeclaAffect(ifccParser::DeclaAffectContext *ctx)
{
    ...
    if (ctx->expression()) {
        ...
        std::cout << "    mov eax, DWORD PTR -"<<expr_index*4<<"[rbp]\n";
        std::cout << "    mov DWORD PTR -"<<symbolsTable.at(variable)*4<<"[rbp], eax\n";
    }
...
antlrcpp::Any CodeGenVisitor::visitIntConst(ifccParser::IntConstContext *ctx)
{
    ...
    symbolsTable.insert(std::make_pair("#" + std::to_string(counterTempVariables), symbolsTable.size()+1));

    std::cout << "    mov DWORD PTR -"<<(symbolsTable.size())*4<<"[rbp], "<< value <<"\n";
...
antlrcpp::Any CodeGenVisitor::visitMult(ifccParser::MultContext *ctx)
{
    ...
    if (ctx->op->getText()[0] == '*') {
        std::cout << "    imul eax, DWORD PTR -"<<expr2_index*4<<"[rbp]\n";
    ...
    std::cout << "    mov DWORD PTR -"<<(symbolsTable.size())*4<<"[rbp], eax\n";
...
antlrcpp::Any CodeGenVisitor::visitUnaryAdd(ifccParser::UnaryAddContext *ctx)
{
    ...
    if (ctx->op->getText()[0] == '+') return (int) expr_index;
